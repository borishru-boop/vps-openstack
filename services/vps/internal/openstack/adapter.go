...
func (a *Adapter) BuildServer(ctx context.Context, serverID string, opts hypervisor.CreateOptions) error {
	...
	if ip, err := a.ensureServerFloatingIP(ctx, serverID); err != nil {
...
func (a *Adapter) GetServer(ctx context.Context, id string) (*hypervisor.Server, error) {
	...
	out := mapServer(srv)
	if _, err := a.ensureServerFloatingIP(ctx, id); err != nil {
		log.Printf("openstack: ensure floating ip for %s: %v", id, err)
	}
...
func (a *Adapter) ReinstallServer(ctx context.Context, id, planID, osTemplateID, rootPassword string, sshKeys []string) error {
	...
	if err := waitServerActive(cli, id); err != nil {
		return err
	}
	if _, err := a.ensureServerFloatingIP(ctx, id); err != nil {
		log.Printf("openstack: reinstall ensure floating ip for %s: %v", id, err)
	}
	return nil
}
...
func mapServer(srv *servers.Server) *hypervisor.Server {
	...
	osVer := 0
	if osBuilt {
		// Mark OS present so worker pollers skip repeat BuildServer on ACTIVE.
		osVer = 1
	}
