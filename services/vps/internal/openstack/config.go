...
type Config struct {
	...
	// FloatingSubnetID pins FIP allocation to a specific external subnet (optional).
...
func LoadConfig() Config {
	cfg := Config{
		...
		Region:            firstEnv("OPENSTACK_REGION", "OPENSTACK_REGION_NAME", "OS_REGION_NAME"),
		...
		DomainName:        firstEnv("OPENSTACK_USER_DOMAIN_NAME", "OPENSTACK_DOMAIN_NAME", "OS_USER_DOMAIN_NAME"),
		...
		FloatingSubnetID:  strings.TrimSpace(os.Getenv("OPENSTACK_FLOATING_SUBNET_ID")),
...
func (c Config) ImageRef(catalogOS string) (string, bool) {
	catalogOS = strings.TrimSpace(catalogOS)
	if ref, ok := c.OSTemplates[catalogOS]; ok && ref != "" {
		return ref, true
	}
	if canonical := catalog.ResolveCatalogOSID(catalogOS); canonical != catalogOS {
		if ref, ok := c.OSTemplates[canonical]; ok && ref != "" {
			return ref, true
		}
	}
	return "", false
...
// SupportsCatalogOS reports whether the portal OS id can be provisioned on OpenStack.
func (c Config) SupportsCatalogOS(catalogOS string) bool {
	_, ok := c.ImageRef(catalogOS)
	return ok
}
