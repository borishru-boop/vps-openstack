...
func (h *Handler) Catalog(w http.ResponseWriter, r *http.Request) {
	...
	if h.store != nil {
		...
		if templates, err := h.activeOSTemplatesForRegion(r.Context(), region); err == nil && len(templates) > 0 {
...
func (h *Handler) ListOSTemplates(w http.ResponseWriter, r *http.Request) {
	region := strings.ToLower(strings.TrimSpace(r.URL.Query().Get("region")))
	if h.store != nil {
		templates, err := h.activeOSTemplatesForRegion(r.Context(), region)
		if err != nil {
	...
	templates := catalog.OSTemplates()
	if hypervisor.RegionUsesOpenStack(region) {
		templates = filterOSTemplatesOpenStack(templates)
	}
	writeJSON(w, http.StatusOK, map[string]any{
		"os_templates": catalog.EnrichOSTemplatesWithSoftware(templates),
	})
...
func (h *Handler) activeOSTemplates(ctx context.Context) ([]catalog.OSTemplate, error) {
	...
	if len(rows) == 0 {
		return catalog.FilterOSTemplatesByMap(hypervisor.LoadConfig().OSTemplates), nil
	}
...
func (h *Handler) activeOSTemplatesForRegion(ctx context.Context, region string) ([]catalog.OSTemplate, error) {
	all, err := h.activeOSTemplates(ctx)
	if err != nil {
		return nil, err
	}
	if !hypervisor.RegionUsesOpenStack(region) {
	...
	return filterOSTemplatesOpenStack(all), nil
}

func filterOSTemplatesOpenStack(all []catalog.OSTemplate) []catalog.OSTemplate {
	cfg := openstack.LoadConfig()
	if len(cfg.OSTemplates) == 0 {
		return nil
	}
	out := make([]catalog.OSTemplate, 0, len(all))
	for _, t := range all {
		if cfg.SupportsCatalogOS(t.ID) {
			out = append(out, t)
		}
	}
	return out
...
func (h *Handler) osTemplateAvailable(ctx context.Context, osID string) bool {
	return h.osTemplateAvailableForRegion(ctx, "", osID)
}

func (h *Handler) osTemplateAvailableForRegion(ctx context.Context, region, osID string) bool {
	...
	if hypervisor.RegionUsesOpenStack(region) {
		return openstack.LoadConfig().SupportsCatalogOS(osID)
	}
	if h.store != nil {
		active, err := h.store.IsOSTemplateActive(ctx, osID)
		return err == nil && active
	}
	_, ok := hypervisor.LoadConfig().OSTemplates[osID]
	return ok
}
...
func (h *Handler) CreateOrder(w http.ResponseWriter, r *http.Request) {
	...
	if !hypervisor.MockEnabled() {
		...
		if hypervisor.RegionUsesOpenStack(req.Region) {
			osCfg := openstack.LoadConfig()
			if !waitlistTier && len(osCfg.PlanFlavors) > 0 && !osCfg.HasExplicitFlavor(plan.ID) {
				writeError(w, http.StatusBadRequest, "plan not available")
				return
			}
		} else {
			cfg := hypervisor.LoadConfig()
			if !waitlistTier && len(cfg.PlanPackages) > 0 && !cfg.HasExplicitPackage(plan.ID) {
				writeError(w, http.StatusBadRequest, "plan not available")
				return
			}
		}
	...
	if !h.osTemplateAvailableForRegion(r.Context(), req.Region, req.OSTemplateID) {
