...
// nodeProvisionableWhere matches online nodes that can accept new orders.
// VirtFusion nodes require vf_enabled; OpenStack nodes use UUID external_id with vf_enabled=false.
const nodeProvisionableWhere = `
	n.status = 'online'
	AND n.external_id IS NOT NULL AND n.external_id <> ''
	AND COALESCE(n.maintenance_mode, false) = false
	AND (
		COALESCE(n.vf_enabled, true) = true
		OR n.external_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
	)
`
